...
def test():
    ...
    lines_path = ResourceService.get("test/sentences/lines_for_axiom.txt")
    ...
    p,q = TermConverter(abt).select([p]),TermConverter(abt).select([q])
    print(p,q)
    p,q = abt.copy_subtree(p,basiccopy),abt.copy_subtree(q,basiccopy)
    temp = line_parser.parse("p ∧ q")
    conclusion = TermConverter(abt).deduce(pre,[temp],name_t={"p":p,"q":q},conclusion=p)
    abt = line_parser.parse(conclusion)
    TermAnalyzer(abt,env).solve()
    Switcher(abt,env).switch()
    line = TermConverter(abt).conclude(pre,abt)
    abt = line_parser.parse(line)
    TermAnalyzer(abt,env).solve()
    Switcher(abt,env).switch()
